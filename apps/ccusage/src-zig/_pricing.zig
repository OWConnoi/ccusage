...
pub fn loadPricing(allocator: std.mem.Allocator, map: *PricingMap, offline: bool, io: std.Io, err_writer: *std.Io.Writer) !void {
    try loadPricingJson(allocator, map, EMBEDDED_PRICING_JSON);
    try putFallbackPricing(map);
    if (offline) return;

    try err_writer.print("WARN  Fetching latest model pricing from LiteLLM...\n", .{});
    ...
    const result = client.fetch(.{
        ...
        try err_writer.print("WARN  Failed to fetch LiteLLM pricing ({s}); using embedded pricing.\n", .{@errorName(err)});
        return;
    };
    if (result.status != .ok) {
        try err_writer.print("WARN  LiteLLM pricing fetch returned {}; using embedded pricing.\n", .{result.status});
        return;
    }

    try loadPricingJson(allocator, map, writer_alloc.written());
    try err_writer.print("INFO  Loaded latest model pricing from LiteLLM.\n", .{});
}
